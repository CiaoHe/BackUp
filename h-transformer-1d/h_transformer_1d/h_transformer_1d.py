...
import torch
from torch import nn, einsum, diagonal, Tensor
...
def masked_aggregate(tensor:Tensor, mask:Tensor=None, dim=-1, average=True):
    if not exists(mask):
...
class HAttention1D(nn.Module):
    ...
    def forward(self, x:Tensor, mask:Tensor=None):
        ...
        # split out heads, and also divide sequence into blks
        ...
        for level in range(num_levels):
            ...
            q = masked_aggregate(q, mask, dim=2, average=True)
            k = masked_aggregate(k, mask, dim=2, average=True)
        ...
        for ind, (q, k, v, mask) in enumerate(reversed(qkvs)):
            ...
            Y_level_and_A = calculate_Y_and_A(q, k, v, mask=S_mask)
            Ys.append(Y_level_and_A)
...
class CausalHAttention(nn.Module):
    def __init__(
        self,
        dim,
        *,
    ...
    def forward(self, x, **kwargs):
        ...
        # calculate number of levels until 2x2
        num_levels = int(log2(pad_to_len // bsz)) - 1
        ...
        for level in range(num_levels):
            ...
            # masked mean for queries and keys, but not for values
            q = masked_aggregate(q, dim = 2)
            k = masked_aggregate(k, dim = 2)
            v = masked_aggregate(v, dim = 2, average=False)
        ...
        def calculate_Y_and_A(q, k, v, mask_right_off_diagonals = False, causal_mask_diagonal=False):
            ...
            S:Tensor = einsum('... i d, ... j d -> ... i j', q, k)
...
class HTransformer1D(nn.Module):
    def __init__(
        ...
        *,
        ...
        attn_class = CausalHAttention if causal else HAttention1D
        attn_kwargs = dict(max_seq_len=max_seq_len) if causal else dict()

        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, attn_class(dim, dim_head=dim_head, heads=heads, block_size=block_size, pos_emb=pos_emb,\
                    **attn_kwargs)),
...
# def test():
#     attn = CausalHAttention(
#         dim = 64,
#         max_seq_len=8192,
#         block_size=128,
#     )

#     x = torch.randn((2, 8000, 64))   # variable sequence length
#     print(attn(x).shape)
