...
def parse_a3m(filename):
    ...
    for line in open(filename,"r"):
        ...
        # remove lowervasae letters and append to msa
...
def parse_hhr(filename, ffindex, idmax=105.0):
    '''
    Out: 
        List[[hits[idx][0],matches,p/100,seqid/100,sim/10]]
         = List[[label(int), matches_position(4-int), prob(float), Identities(float), Template_Neff(float)]]
    '''
...
def parse_pdb(filename):
    ...
    # indices or residues observed in the structure
...
def parse_pdb_lines(lines):
    '''
    Return:
        xyz: Array #(L, 14, 3), only record CA
        mask: Array #(L, 14,)
        idx_S: Array #(L, )
    '''

    # indices or residues observed in the structure
    idx_s = np.array([int(l[22:26]) for l in lines if l[:4] == 'ATOM' and l[12:16].strip() == 'CA'])
    ...
    for l in lines:
        if l[:4] != 'ATOM':
            continue
        resNo, atom, aa = int(l[22:26]), l[12:16], l[17:20] # (1, ' N  ', 'MET')
    ...
    # save atom mask
    mask = np.logical_not(np.isnan(xyz[...,0])) # 如果x轴没有记录坐标（以x为例，当然y/z都不会有数据)
...
def parse_templates(ffdb, hhr_fn, atab_fn, n_templ=10):
    '''
    Args:
        ffdb: Database handler
        hhr_fn: hhr filename/handler
        atab_fn: atab filename/handler
    ...
    # process tabulated hhsearch output to get matched positions and positional scores
    inflie = atab_fn
    hits = []
    '''
    hits: List[ [key, [[i,j],..], [[score, SS, probab],...] ]
    '''
    for l in open(inflie, "r").readlines():
        if l[0] == '>':
            key = l[1:].split()[0]
            hits.append([key, [], []])
    ...
    # [Probab, E-value, Score, Aligned_cols, Identities, Similarity, Sum_probs, Template_Neff]
    lines = open(hhr_fn, "r").readlines()
    pos = [i+1 for i,l in enumerate(lines) if l[0]=='>']
    '''
    hits: List[ [key, [[i,j],..], [[score, SS, probab],...], [Probab, E-value, Score, Aligned_cols, Identities, Similarity, Sum_probs, Template_Neff]]
    '''
    ...
    for hi in hits:
        ...
        '''
        hits: List[ *s* [key, [[i,j],..], [[score, SS, probab],...], \
            [Probab, E-value, Score, Aligned_cols, Identities, Similarity, Sum_probs, Template_Neff], \
            xyz, mask, np.array(idx_s)] *e* ]
        '''
    ...
    hit:[
            key,
            [[i,j],..], # for each atom (Lx2)
            [[score, SS, probab],...], # for each atom (Lx3)
            [Probab, E-value, Score, Aligned_cols, Identities, Similarity, Sum_probs, Template_Neff] # (8,)
            xyz,  # Array (L, 14, 3)
            mask, # Array (L, 14,)
            ids,  # Array (L, )
    ]
    ...
    for data in hits:
        ...
        qmap.append(np.stack([qi[sel1]-1,[counter]*ncol],axis=-1)) # [len(sel), 2]
        counter += 1

    xyz = np.vstack(xyz).astype(np.float32) # (N, L, 14, 3)
    qmap = np.vstack(qmap).astype(np.long)  # (N, L, 2)
    f0d = np.vstack(f0d).astype(np.float32) # (N, 8)
    f1d = np.vstack(f1d).astype(np.float32) # (N, L, 3)
    ids = ids                               # (N, L)
...
def read_templates(qlen, ffdb, hhr_fn, atab_fn, n_templ=10):
    ...
    Return:
        ...
        f0d: Tensor (n, 3)
    '''
    xyz_t, qmap, t0d, t1d, ids = parse_templates(ffdb, hhr_fn, atab_fn, n_templ)
