...
class Predictor():
    def __init__(self, model_dir = None, use_cpu=False) -> None:
        ...
        self.device = torch.device("cuda") if torch.cuda.is_available() and (not use_cpu) else \
            torch.device("cpu")
    ...
    def load_model(self, model_name, suffix='e2e'):
        ...
        if not os.path.exists(chk_fn):
            print(f'Not exists checkpoint in {chk_fn}')
            return False
    ...
    def predict(
        ...
        msa = parse_a3m(a3m_fn) # still numpy
        N,L = msa.shape
        msa = torch.tensor(msa).long().view(1, -1, L) # (1,N,L)
        ...
        # template features
        xyz_t = xyz_t.float().unsqueeze(0) # (1, n, L, 3, 3) n = tmpl_num, L = nres
        t1d = t1d.float().unsqueeze(0) # (1, n, L, 3)
        t0d = t0d.float().unsqueeze(0) # (1, n, 3)
        t2d = xyz_to_t2d(xyz_t, t0d) # (1, L, L, 10)
        ...
        with torch.no_grad():
            ...
            if L > window * 2:
                # 需要截取2*window长度的seq
                prob_s = [np.zeros((L,L,NBIN[i]), dtype=np.float32) for i in range(4)] # 4 list [L, L, bins]
                count_1d = np.zeros((L,), dtype=np.float32) # [L,]
                count_2d = np.zeros((L,L), dtype=np.float32)# [L,L]
                node_s = np.zeros((L,MODEL_PARAM['d_msa']), dtype=np.float32) # [L, 384]
                #
                ...
                for i in range(ngrids):
                    for j in range(i,ngrids):
                        start_1 = grids[i]
                        end_1 = min(start_1+window, L)
                        start_2 = grids[j]
                        end_2 = min(start_2+window, L)
                        sel = np.zeros((L)).astype(np.bool)
                        sel[start_1:end_1] = True
                        sel[start_2:end_2] = True

                        input_msa = msa[:, :, sel]
                        mask = torch.sum(input_msa==20, dim=-1) < 0.5*sum(sel) # remove too gappy seqs
                        ...
                        input_t2d = t2d[:,:,sel][:,:,:,sel].to(self.device) # (B, T, L, L, 10)
                        ...
                        for i_logit, logit in enumerate(logit_s): # 4 group of logit
                            prob = self.active_fn(logit)
                ...
                prob_in = np.concatenate(prob_s, axis=-1) # (L, L, all_bins)
                node_s = node_s / count_1d[:,None] #(L,384)
                ...
                prob_s = list()
                # logit_s: [dist, omega, theta, phi] (-1,L,L)
        ...
        # write pdb
        self.write_pdb(seq[0], xyz, idx_pdb[0], Bfacts=lddt[0], prefix="%s_init"%(out_prefix))
        ...
        xyz = TRF.fold(xyz, batch=15, lr=0.1, nsetps=200)
