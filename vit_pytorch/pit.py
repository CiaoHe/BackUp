...
class Attention(nn.Module):
    def __init__(
        ...
        project_out = not (heads == 1 and dim_head == dim) # 如果只有1个头不需要project
    ...
    def forward(self, x):
        ...
        q,k,v= map(lambda t: rearrange('b n (h d) -> b h n d', h = h), qkv)
...
class DepthWiseConv2d(nn.Module):
    def __init__(
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(dim_out, dim_out, kernel_size=1, bias=bias) # 1*1 conv
        )
    def forward(self, x):
        return self.net(x)
...
class Pool(nn.Module):
    def __init__(self, dim):
        ...
        self.cls_ff = nn.Linear(dim, dim*2) # FC-layer for cls-token
    ...
    def __init__(
        ...
        *,
        ...
        self.to_patch_embedding = nn.Sequential(
            nn.Unfold(kernel_size=patch_size, stride=patch_size//2), # over-lapping split
            Rearrange('b c n -> b n c'),
            nn.Linear(patch_dim, dim)
        )

        output_size = conv_output_size(image_size=image_size, kernel_size=patch_size, stride=patch_size//2)
