...
class Attention(nn.Module):
    def __init__(
        ...
        self.attn_bias = nn.Embedding(fmap_size**2, heads)
    ...
    def apply_attn_bias(self, fmap:Tensor):
        bias = self.attn_bias(self.pos_indices)
    ...
    def forward(self, x:Tensor):
        ...
        dots = self.apply_attn_bias(dots)
    ...
    def __init__(
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Attention(dim, fmap_size=fmap_size, heads=heads, dim_key=dim_key, dim_value=dim_value, dropout=dropout, dim_out=dim_out, downsample=downsample),
...
class LeViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        num_classes,
        dim,   # better be tuple
        depth, # better be tuple
        heads, # better be tuple
        mlp_mult,
        stages = 3,
        dim_key = 32,
        dim_value = 64,
        ...
        fmap_size = image_size // (2**4) # e.g. 224 -> 14
        layers = []

        for ind, dim, depth, heads in zip(range(stages), dims, depths, layer_heads):
            is_last = ind == (stages-1)
            layers.append(Transformer(dim, fmap_size=fmap_size, depth=depth, heads=heads, dim_key=dim_key, dim_value=dim_value, mlp_mult=mlp_mult, dropout=dropout, downsample=False))

            if not is_last:
                next_dim = dims[ind+1]
                layers.append(Transformer(dim, fmap_size=fmap_size, depth=1, heads=heads*2, dim_key=dim_key, dim_value=dim_value, mlp_mult=2, dropout=dropout, dim_out=next_dim, downsample=True))
                # update fmap_size
        ...
        self.distill_head = nn.Linear(dims[-1], num_distill_classes) if exists(num_distill_classes) else always(None)
        self.mlp_head = nn.Linear(dims[-1], num_classes)
