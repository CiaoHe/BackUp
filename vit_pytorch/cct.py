...
def _cct(num_layers, num_heads, mlp_ratio, embedding_dim, kernel_size=3, \
    stride=None, padding=None, *args, **kwargs):
    stride = stride if exists(stride) else max(1, (kernel_size//2)-1)
    padding = padding if exists(padding) else max(1, (kernel_size//2))
    return CCT(
...
class Tokenizer(nn.Module):
    def __init__(
        ...
        n_filter_list = [n_input_channels] + \
            [in_planes for _ in range(n_conv_layers-1)] + [n_output_channels]
        
        self.conv_layers = nn.ModuleList([])
        for i in range(n_conv_layers):
            self.conv_layers.append(nn.Sequential(
                nn.Conv2d(n_filter_list[i], n_filter_list[i+1], kernel_size=kernel_size,
...
class TransformerClassifier(nn.Module):
        ...
        *args, **kwargs
        ...
        if positional_embedding != 'none':
            if positional_embedding == 'learnable':
                self.positional_emb = nn.Parameter(torch.zeros(1, sequence_length, self.embedding_dim))
                nn.init.trunc_normal_(self.positional_emb, std=0.2)
            elif positional_embedding == 'sine':
                self.positional_emb = nn.Parameter(self.sinusoidal_embedding(sequence_length, self.embedding_dim), requires_grad=False)
        else:
        ...
        self.blocks = nn.ModuleList([
            TransformerEncoderLayer(d_model=embedding_dim, nhead=num_heads, dim_feedforward=dim_feedforward, dropout=dropout_rate, attention_dropout=attention_dropout, drop_path_rate=dpr[i])
    ...
    def sinusoidal_embedding(n_channels, dim):
        ...
        return pe.unsqueeze(0) # (1, channels, dim)
    ...
    def forward(self, x):
        ...
        if not self.seq_pool:
            cls_token = rearrange(self.class_emb, '() n d -> b n d', b = b)
        ...
        if self.seq_pool:
            ...
            x_L = F.softmax(x, dim=1).transpose(-1,-2)
            z = einsum('b () n, b n d -> b () d', x_L, x)
            z = z.squeeze(-2)
        else:
            z = x[:,0]
